...
async fn setup(
    ...
    mint: Keypair,
    init_params: ExtensionInitializationParams,
) {
    test_context
        .init_token_with_mint_keypair_and_freeze_authority(mint, vec![init_params], None)
        .await
        .unwrap();
}
...
async fn success_initialize_member() {
    ...
    let payer_pubkey = context.lock().await.payer.pubkey();
    ...
    let error = member1_token_context
        ...
        .token_group_initialize_member_with_rent_transfer(
            ...
            &group_mint_keypair.pubkey(),
            &group_update_authority.pubkey(),
            &[&not_mint_authority, &group_update_authority],
        )
        .await
        .unwrap_err();
    assert_eq!(
...
async fn fail_without_signatures() {
    ...
    let error = member_token_context
        .token
        .process_ixs(&[instruction], &[&group_update_authority])
        .await
        .unwrap_err();
    assert_eq!(
        error,

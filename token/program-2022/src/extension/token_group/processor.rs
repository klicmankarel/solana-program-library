...
pub fn process_initialize_group(
    ...
    let group_extension_len =
        ExtensionType::try_calculate_account_len::<Mint>(&[ExtensionType::TokenGroup])?;
    let new_account_len = group_info
        .data_len()
        .checked_add(group_extension_len)
        .ok_or(TokenError::Overflow)?;
    group_info.realloc(new_account_len, false)?;
    let mut buffer = group_info.try_borrow_mut_data()?;
    let mut state = StateWithExtensionsMut::<Mint>::unpack(&mut buffer)?;
    let group = state.init_extension::<TokenGroup>(false)?;
...
pub fn process_initialize_group_member(
    ...
    // Allocate a TLV entry for the space and write it in
    let member_extension_len =
        ExtensionType::try_calculate_account_len::<Mint>(&[ExtensionType::TokenGroupMember])?;
    let new_account_len = member_info
        .data_len()
        .checked_add(member_extension_len)
        .ok_or(TokenError::Overflow)?;
    member_info.realloc(new_account_len, false)?;
    let mut buffer = member_info.try_borrow_mut_data()?;
    let mut state = StateWithExtensionsMut::<Mint>::unpack(&mut buffer)?;
    let member = state.init_extension::<TokenGroupMember>(false)?;
